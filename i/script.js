document.addEventListener('DOMContentLoaded', () => {
    ...
    const DEFAULT_LINE_COLOR = '#cbd5e1'; // 淺灰線
    const DOT_COLOR = '#475569'; // 深灰點
    const DOT_TEXT_COLOR = '#ffffff'; // 白色文字
    ...
    function handleWorkerMessage(e) {
        ...
        if (type === 'bestMoveFound') {
            ...
            const mainDotA = dots[dotA.r][dotA.c];
            const mainDotB = dots[dotB.r][dotB.c];
            currentHint = { p1: mainDotA, p2: mainDotB };
            drawCanvas(); 
            setTimeout(() => {
                if(currentHint) {
                    currentHint = null;
    ...
    function updateWinRateUI(rate) {
        ...
        if (finalPercent < 50) {
            ...
            winRateValue.textContent = `AI/P2 優勢 ${finalPercent}%`;
    ...
    function drawCanvasInternal() {
        ...
        if (currentHint) {
            ctx.beginPath();
            ctx.moveTo(currentHint.p1.x, currentHint.p1.y);
            ctx.lineTo(currentHint.p2.x, currentHint.p2.y);
    ...
    function requestHint() {
        if (isGameOver() || isBatchRunning || isAnimating) return;
        if (gameMode === 'cvc' || (gameMode === 'pvc' && currentPlayer === 2)) {
            alert("現在是電腦思考時間，無法使用提示功能。");
            return;
        }
