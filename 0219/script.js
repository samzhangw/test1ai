...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 遊戲狀態
    let currentPlayer = 1;
    let scores = { 1: 0, 2: 0, p1Dots: 0, p2Dots: 0 }; 
    let dots = [];
    let lines = {};
    let squares = [];
    ...
    function initGame() {
        ...
        if (isBatchRunning && batchInitialState) {
            try {
                ...
                scores = state.scores;
                // 批次還原時，確保 scores 結構正確
                if (scores.p1Dots === undefined) scores.p1Dots = 0;
                if (scores.p2Dots === undefined) scores.p2Dots = 0;
    ...
    function initGameStandard() {
        ...
        // 初始化分數與點數總和
        scores = { 1: 0, 2: 0, p1Dots: 0, p2Dots: 0 };
        dots = [];
        lines = {};
        squares = [];
        selectedDot1 = null;
        ...
        for (let r = 0; r < gridRows - 1; r++) {
            for (let c = 0; c < gridCols - 1; c++) {
                ...
                const v2 = `V_${r},${c + 1}`;
                squares.push({
                    lineKeys: [h1, h2, v1, v2], // [Top, Bottom, Left, Right]
                    x: dots[r][c].x,
                    y: dots[r][c].y,
                    size: DOT_SPACING,
                    filled: false, 
    ...
    function drawCanvasInternal() {
        ...
        squares.forEach(sq => {
            if (sq.filled) {
                ...
                // 2.3 繪製四方支架 (灰藍色) - ⚠️ 修正：線條直接連到邊緣
                ctx.strokeStyle = '#95a5a6';
                ctx.lineWidth = 3;
                ctx.lineCap = 'round';

                // Top (sq.lineKeys[0])
                if (lines[sq.lineKeys[0]] && lines[sq.lineKeys[0]].players.length === 0) {
                    ctx.beginPath(); 
                    ctx.moveTo(cx, cy); 
                    ctx.lineTo(cx, sq.y); // 直達頂邊
                    ctx.stroke();
                }
                // Bottom (sq.lineKeys[1])
                if (lines[sq.lineKeys[1]] && lines[sq.lineKeys[1]].players.length === 0) {
                    ctx.beginPath(); 
                    ctx.moveTo(cx, cy); 
                    ctx.lineTo(cx, sq.y + sq.size); // 直達底邊
                    ctx.stroke();
                }
                // Left (sq.lineKeys[2])
                if (lines[sq.lineKeys[2]] && lines[sq.lineKeys[2]].players.length === 0) {
                    ctx.beginPath(); 
                    ctx.moveTo(cx, cy); 
                    ctx.lineTo(sq.x, cy); // 直達左邊
                    ctx.stroke();
                }
                // Right (sq.lineKeys[3])
                if (lines[sq.lineKeys[3]] && lines[sq.lineKeys[3]].players.length === 0) {
                    ctx.beginPath(); 
                    ctx.moveTo(cx, cy); 
                    ctx.lineTo(sq.x + sq.size, cy); // 直達右邊
                    ctx.stroke();
                }
    ...
    function saveState() {
        const state = {
            ...
            scores: { ...scores }, // 會自動複製 p1Dots, p2Dots
    ...
    function renderHistory() {
        ...
        if (moveHistory.length === 0) {
            historyListElement.innerHTML = '<div class="history-empty">暫無紀錄</div>';
            return;
        }
        ...
        for (let i = moveHistory.length - 1; i >= 0; i--) {
            ...
            // 判斷是誰的移動來設定顏色
            const playerClass = move.player.includes("1") ? "history-p1" : "history-p2";
            
            // 處理舊資料或 undefined 的情況
            const vA = move.valA !== undefined ? move.valA : '?';
            const vB = move.valB !== undefined ? move.valB : '?';
            const sum = move.moveSum !== undefined ? move.moveSum : (typeof vA === 'number' && typeof vB === 'number' ? vA + vB : 0);

            div.innerHTML = `
                <div>
                    <span class="history-turn">#${move.turn}</span>
                    <span class="${playerClass}">${move.player}</span>
                </div>
                <div class="history-mid">
                    <div class="history-move">${move.move}</div>
                    <div class="history-equation">${vA} + ${vB}</div>
                </div>
                <div class="history-sum">+${sum}</div>
            `;
    ...
    function confirmLine() {
        ...
        // --- 計算連線點數 (新增邏輯) ---
        const valA = dotA.number || 0;
        const valB = dotB.number || 0;
        const moveSum = valA + valB;
        if (currentPlayer === 1) scores.p1Dots += moveSum;
        else scores.p2Dots += moveSum;
    ...
    // [修改] 確保 turnCounter 的計算邏輯：得分且續手時，不增加計數
    function logMove(dotA, dotB, scored, valA, valB, moveSum) {
        const moveData = {
            turn: turnCounter,
            ...
            valA: valA, // 紀錄點 A 數值
            valB: valB, // 紀錄點 B 數值
            moveSum: moveSum, // 紀錄總和
            scoreP1: scores[1],
            scoreP2: scores[2],
            p1TotalDots: scores.p1Dots, 
            p2TotalDots: scores.p2Dots
        };
        moveHistory.push(moveData); 
        
        // 更新歷史紀錄 UI
        if (!isBatchRunning) renderHistory();
    ...
    function executeAIMove(dotA, dotB) {
        ...
        if (isChainSearching) {
            ...
            if (chainLen >= chainThreshold) {
                saveFoundChain(chainLen); 
                
                if (foundChains.length >= searchTargetCount) {
                    finishChainSearch(false); 
                    return; 
                } else {
                    setTimeout(initGame, 10);
                    return; 
